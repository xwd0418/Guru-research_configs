...
#     # only H + C
#     sed -i "4s/.*/  name: xwd-large-model--only-1d-trail${seed}-i/" jobs/morgan_fp_job.yml
...
#     kubectl $mode  -f  jobs/morgan_fp_job.yml
    ...
    # # only HSQC + C
    # sed -i "4s/.*/  name: xwd-large-model-hsqc-and-c-trail${seed}-i/" jobs/morgan_fp_job.yml
...
#     # only HSQC + H
#     sed -i "4s/.*/  name: xwd-large-model-hsqc-and-h-trail${seed}-i/" jobs/morgan_fp_job.yml
...
#     kubectl $mode  -f  jobs/morgan_fp_job.yml
    ...
    # only HSQC 
    sed -i "4s/.*/  name: xwd-large-model--only-hsqc-trail${seed}-i/" jobs/morgan_fp_job.yml
    ...
    # only HSQC  with hyun FP
    sed -i "4s/.*/  name: xwd-large-model--only-hsqc-trail${seed}-hyun-fp/" jobs/morgan_fp_job.yml
    sed -i "41s/.*/              python train_ranker_transformer.py transformer_2d1d --foldername all_data_possible --random_seed ${seed}     --dim_model 784 --dim_coords 365 365 54 --bs 32 --accumulate_grad_batches_num 4 --ff_dim  3072 --warm_up_steps 8000  --expname only_hsqc_hyun_FP_trial_${seed} --use_oneD_NMR_no_solvent false  --scheduler attention  --wavelength_bounds 0.01 400.0 --wavelength_bounds 0.01 20.0 --FP_choice HYUN_FP/" jobs/morgan_fp_job.yml
    kubectl $mode  -f  jobs/morgan_fp_job.yml


    # # testing
    # sed -i "4s/.*/  name: xwd-large-model-test/" jobs/morgan_fp_job.yml
    # sed -i "41s/.*/              python train_ranker_transformer.py transformer_2d1d --foldername test_all_data_possible --random_seed ${seed}     --dim_model 784 --dim_coords 365 365 54 --bs 32 --accumulate_grad_batches_num 4 --ff_dim  3072 --warm_up_steps 8000  --only_oneD_NMR true --combine_oneD_only_dataset true --expname only_1d_trial_${seed}    --scheduler attention  --wavelength_bounds 0.01 400.0 --wavelength_bounds 0.01 20.0 --FP_choice pick_entropy_r2 --epochs 2 /" jobs/morgan_fp_job.yml
    # kubectl $mode  -f  jobs/morgan_fp_job.yml
done