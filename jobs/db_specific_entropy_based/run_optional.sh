...
for seed in 2 ; do
    ...
    sed -i "4s/.*/  name: wangdong-optional-model-${seed}-run-jitter-optional-mw-new-embed/" jobs/morgan_fp_job.yml
    sed -i "41s/.*/              python train_ranker_transformer.py transformer_2d1d --foldername flexible_models_jittering_flexible_MW_flexible_normal_hsqc_separate_embedding  --expname r0_r6_trial_${seed} --optional_inputs true --combine_oneD_only_dataset true --random_seed ${seed} --FP_choice ${FP_version}_FP_R_${r}  --out_dim $out_dim --optional_MW true  --jittering 1/" jobs/morgan_fp_job.yml
    kubectl $mode  -f  jobs/morgan_fp_job.yml
